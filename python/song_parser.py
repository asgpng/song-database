...
def is_a_chord(test):
    ...
    # regex = re.compile(r"""[CDEFGAB](?![ac-z])(?:#|##|b|bb)?(?:maj|min|m|sus|aug|dim)?[0-9]?""")
...
def html_export(lines):
    ...
    while i < len(sheet_music):
        ...
        if found_header:
            ...
            if line[0] in headings:
                ...
                #     # out_text += '<div class="lyrics">' + line[1] + '</div>\n'
                # elif i == 1:
    ...
    # print metadata
    # out_text = re.sub('#(<br */?>\s*)+#i', '<br />', out_text)
