...
class Song extends CI_Model {
  ...
  function song_exist_id($song_id) {
  ...
  function get_header($id) {
    ...
    if ($song->title != "" && $song->author != "") {
      $output .= '<i>' . $song->title . '</i> by ' . $song->author;
    }
    if ($song->year != 0) {
      $output .= ', &copy; ' . $song->year . ' ';
    }
    if ($song->producer != "") {
      $output .= $song->producer . ' ';
    }
    if ($song->ccli != 0) {
      $output .= "CCLI# " . $song->ccli;
    }
    return $output;
  ...
  function update_text($song_id, $new_text) {
    $this->db->query("UPDATE songs SET text='$new_text' where id = '$song_id';");
  }
