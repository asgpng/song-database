...
class CAS_Client
{
    ...
    */
    ...
    public function printHTMLFooter()
    {
        ...
        $this->_htmlFilterOutput(
            ...
            ('<hr><address>CAS __PHPCAS_VERSION__ '
            .$lang->getUsingServer()
    ...
    */
    ...
    /**
     * @var boolean $_clearTicketsFromUrl; If true, CAS will clear session
    ...
    */
     ...
     * @param bool   $changeSessionID Allow CAS to change the session_id (Single Sign Out/handleLogoutRequests is based on that change)
     *
    ...
    public function __construct(
        ...
        if (session_id()=="" && !$this->_isLogoutRequest()) {
            CAS :: trace("Starting a new session");
            session_start();
        }
        ...
        if ($this->isProxy()) {
            ...
            if (!isset($_SESSION['CAS']['service_cookies'])) {
                $_SESSION['CAS']['service_cookies'] = array();
            }
            $this->_serviceCookieJar = new CAS_CookieJar($_SESSION['CAS']['service_cookies']);
        }
        ...
        default:
            CAS::error(
                ...
                .'\') is not supported by CAS '.CAS::getVersion()
            );
        }
        ...
        if ( $this->_isCallbackMode() ) {
            ...
            if ( !$this->_isHttps() ) {
                CAS::error('CAS proxies must be secured to use CAS; PGT\'s will not be received from the CAS server');
            }
        } else {
    ...
    /**
     * Set a parameter whether to allow CAS to change session_id
     *
     * @param bool $allowed allow CAS to change session_id
     *
     * @return void
     */
     ...
     * @attention client applications should use CAS::getAttributes().
     *
     * @hideinitializer
     */
    ...
    public function renewAuthentication()
    {
        ...
        if (isset( $_SESSION['CAS']['auth_checked'])) {
            unset($_SESSION['CAS']['auth_checked']);
        }
        if ( $this->isAuthenticated() ) {
            CAS::trace('user already authenticated; renew');
    ...
    public function checkAuthentication()
    {
        ...
        if ( $this->isAuthenticated() ) {
            ...
            unset($_SESSION['CAS']['auth_checked']);
            $res = true;
        } else if (isset($_SESSION['CAS']['auth_checked'])) {
            ...
            if (!isset($_SESSION['CAS']['unauth_count'])) {
                $_SESSION['CAS']['unauth_count'] = -2; // uninitialized
            }

            if (($_SESSION['CAS']['unauth_count'] != -2
                && $this->_cache_times_for_auth_recheck == -1)
                || ($_SESSION['CAS']['unauth_count'] >= 0
                && $_SESSION['CAS']['unauth_count'] < $this->_cache_times_for_auth_recheck)
            ) {
                $res = false;

                if ($this->_cache_times_for_auth_recheck != -1) {
                    $_SESSION['CAS']['unauth_count']++;
                    CAS::trace(
                        'user is not authenticated (cached for '
                        .$_SESSION['CAS']['unauth_count'].' times of '
                        .$this->_cache_times_for_auth_recheck.')'
                    );
                } else {
                    CAS::trace('user is not authenticated (cached for until login pressed)');
                }
            } else {
                $_SESSION['CAS']['unauth_count'] = 0;
                $_SESSION['CAS']['auth_checked'] = true;
    ...
    public function isAuthenticated()
    {
        ...
        if ( $this->_wasPreviouslyAuthenticated() ) {
            if ($this->hasTicket()) {
                ...
                CAS::trace('ticket was present and will be discarded, use renewAuthenticate()');
                if ($this->_clearTicketsFromUrl) {
                    ...
                    CAS::trace('Already authenticated, but skipping ticket clearing since setNoClearTicketsFromUrl() was used.');
                    $res = true;
                }
            } else {
            ...
            if ($this->hasTicket()) {
                ...
                case CAS_VERSION_1_0:
                    ...
                    CAS::trace('CAS 1.0 ticket `'.$this->getTicket().'\' is present');
                    $this->validateCAS10($validate_url, $text_response, $tree_response); // if it fails, it halts
                    CAS::trace('CAS 1.0 ticket `'.$this->getTicket().'\' was validated');
                    $_SESSION['CAS']['user'] = $this->getUser();
                    $res = true;
                    $logoutTicket = $this->getTicket();
                    break;
                case CAS_VERSION_2_0:
                    // if a Proxy Ticket was given, validate it
                    CAS::trace('CAS 2.0 ticket `'.$this->getTicket().'\' is present');
                    $this->validateCAS20($validate_url, $text_response, $tree_response); // note: if it fails, it halts
                    CAS::trace('CAS 2.0 ticket `'.$this->getTicket().'\' was validated');
                    if ( $this->isProxy() ) {
                        $this->_validatePGT($validate_url, $text_response, $tree_response); // idem
                        CAS::trace('PGT `'.$this->_getPGT().'\' was validated');
                        $_SESSION['CAS']['pgt'] = $this->_getPGT();
                    }
                    $_SESSION['CAS']['user'] = $this->getUser();
                    if ($this->hasAttributes()) {
                        $_SESSION['CAS']['attributes'] = $this->getAttributes();
                    }
                    $proxies = $this->getProxies();
                    if (!empty($proxies)) {
                        $_SESSION['CAS']['proxies'] = $this->getProxies();
                    }
                    $res = true;
                    $logoutTicket = $this->getTicket();
                    break;
                case SAML_VERSION_1_1:
                    ...
                    CAS::trace('SAML 1.1 ticket `'.$this->getTicket().'\' is present');
                    $this->validateSA($validate_url, $text_response, $tree_response); // if it fails, it halts
                    CAS::trace('SAML 1.1 ticket `'.$this->getTicket().'\' was validated');
                    $_SESSION['CAS']['user'] = $this->getUser();
                    $_SESSION['CAS']['attributes'] = $this->getAttributes();
                    $res = true;
                    $logoutTicket = $this->getTicket();
    ...
    {
        return !empty($_SESSION['CAS']['user']);
    }
    ...
    private function _wasPreviouslyAuthenticated()
    {
        ...
        if ( $this->isProxy() ) {
            ...
            if ( $this->isSessionAuthenticated() && !empty($_SESSION['CAS']['pgt']) ) {
                // authentication already done
                $this->_setUser($_SESSION['CAS']['user']);
                if (isset($_SESSION['CAS']['attributes'])) {
                    $this->setAttributes($_SESSION['CAS']['attributes']);
                }
                $this->_setPGT($_SESSION['CAS']['pgt']);
                CAS::trace('user = `'.$_SESSION['CAS']['user'].'\', PGT = `'.$_SESSION['CAS']['pgt'].'\'');

                // Include the list of proxies
                if (isset($_SESSION['CAS']['proxies'])) {
                    $this->_setProxies($_SESSION['CAS']['proxies']);
                    CAS::trace('proxies = "'.implode('", "', $_SESSION['CAS']['proxies']).'"');
                }

                $auth = true;
            } elseif ( $this->isSessionAuthenticated() && empty($_SESSION['CAS']['pgt']) ) {
                // these two variables should be empty or not empty at the same time
                CAS::trace('username found (`'.$_SESSION['CAS']['user'].'\') but PGT is empty');
                // unset all tickets to enforce authentication
                unset($_SESSION['CAS']);
                $this->setTicket('');
            } elseif ( !$this->isSessionAuthenticated() && !empty($_SESSION['CAS']['pgt']) ) {
                // these two variables should be empty or not empty at the same time
                CAS::trace('PGT found (`'.$_SESSION['CAS']['pgt'].'\') but username is empty');
                // unset all tickets to enforce authentication
                unset($_SESSION['CAS']);
                $this->setTicket('');
            } else {
            ...
            if ( $this->isSessionAuthenticated() ) {
                ...
                CAS::trace('user = `'.$_SESSION['CAS']['user'].'\'');
    ...
    public function handleLogoutRequests($check_client=true, $allowed_clients=false)
    {
        ...
        if (!$this->getChangeSessionID() && is_null($this->_signoutCallbackFunction)) {
            CAS::trace("CAS can't handle logout requests if it is not allowed to change session_id.");
        }
        ...
        if ($check_client) {
            ...
            foreach ($allowed_clients as $allowed_client) {
                if (($client == $allowed_client) or ($client_ip == $allowed_client)) {
                    CAS::trace("Allowed client '".$allowed_client."' matches, logout request is allowed");
                    $allowed = true;
                    break;
                } else {
                    CAS::trace("Allowed client '".$allowed_client."' does not match");
                }
            }
        } else {
        ...
        if ($allowed) {
            ...
            // If CAS is managing the session_id, destroy session thanks to session_id.
    ...
    public function validateSA(&$validate_url,&$text_response,&$tree_response)
    {
        ...
        case SAML_VERSION_1_1:
            ...
            if (!($tree_response = $dom->documentElement)) {
                ...
                CAS::trace('bad XML root node (should be `Envelope\' instead of `'.$tree_response->localName.'\'');
                throw new CAS_AuthenticationException(
                    $this, 'SA not validated', $validate_url,
                    false/*$no_response*/, true/*$bad_response*/,
                    $text_response
    ...
    */
    /**
     * each PHP script using CAS in proxy mode is its own callback to get the
    ...
    private function _callback()
    {
        ...
        if (preg_match('/PGTIOU-[\.\-\w]/', $_GET['pgtIou'])) {
            if (preg_match('/[PT]GT-[\.\-\w]/', $_GET['pgtId'])) {
                $this->printHTMLHeader('CAS callback');
                $pgt_iou = $_GET['pgtIou'];
                $pgt = $_GET['pgtId'];
                ...
                $this->printHTMLFooter();
                CAS::traceExit("Successfull Callback");
            } else {
                CAS::error('PGT format invalid' . $_GET['pgtId']);
                CAS::traceExit('PGT format invalid' . $_GET['pgtId']);
            }
        } else {
            CAS::error('PGTiou format invalid' . $_GET['pgtIou']);
            CAS::traceExit('PGTiou format invalid' . $_GET['pgtIou']);
        }
    ...
    */
    ...
    public function setPGTStorage($storage)
    {
        ...
        if ( !($storage instanceof CAS_PGTStorage_AbstractStorage) ) {
            CAS::error('Invalid PGT storage object');
        }
    ...
    */
    ...
    public function retrievePT($target_service,&$err_code,&$err_msg)
    {
        ...
        if ( !$this->_readURL($cas_url, $headers, $cas_response, $err_msg) ) {
            CAS::trace('could not open URL \''.$cas_url.'\' to validate ('.$err_msg.')');
    ...
    private function _readURL($url, &$headers, &$body, &$err_msg)
    {
        ...
        if (empty($this->_cas_server_ca_cert) && !$this->_no_cas_server_validation) {
            CAS::error('one of the methods CAS::setCasServerCACert() or CAS::setNoCasServerValidation() must be called.');
        }
        ...
        if ( $success_elements->item(0)->getElementsByTagName("attributes")->length != 0) {
            ...
            if ($attr_nodes->item(0)->hasChildNodes()) {
                ...
                foreach ($attr_nodes->item(0)->childNodes as $attr_child) {
                    CAS :: trace("Attribute [".$attr_child->localName."] = ".$attr_child->nodeValue);
            ...
            foreach ($childnodes as $attr_node) {
                ...
                default:
                    if (strlen(trim($attr_node->nodeValue))) {
                        CAS :: trace("Attribute [".$attr_node->localName."] = ".$attr_node->nodeValue);
            ...
            if (!$firstAttr->hasChildNodes() && $firstAttr->hasAttribute('name') && $firstAttr->hasAttribute('value')) {
                ...
                foreach ($attr_nodes as $attr_node) {
                    if ($attr_node->hasAttribute('name') && $attr_node->hasAttribute('value')) {
                        CAS :: trace("Attribute [".$attr_node->getAttribute('name')."] = ".$attr_node->getAttribute('value'));
    ...
    private function _renameSession($ticket)
    {
        ...
        if ($this->getChangeSessionID()) {
            if (!empty($this->_user)) {
                ...
                CAS :: error('Session should only be renamed after successfull authentication');
            }
        } else {
            CAS :: trace("Skipping session rename since CAS is not handling the session.");
        }
        CAS::traceEnd();
    }
    ...
    private function _rebroadcast($type)
    {
        ...
        for ($i = 0; $i < sizeof($this->_rebroadcast_nodes); $i++) {
            if ((($this->_getNodeType($this->_rebroadcast_nodes[$i]) == self::HOSTNAME) && !empty($dns) && (stripos($this->_rebroadcast_nodes[$i], $dns) === false)) || (($this->_getNodeType($this->_rebroadcast_nodes[$i]) == self::IP) && !empty($ip) && (stripos($this->_rebroadcast_nodes[$i], $ip) === false))) {
                CAS::trace('Rebroadcast target URL: '.$this->_rebroadcast_nodes[$i].$_SERVER['REQUEST_URI']);
                $className = $this->_requestImplementation;
                $request = new $className();
                ...
                CAS::trace('Rebroadcast not sent to self: '.$this->_rebroadcast_nodes[$i].' == '.(!empty($ip)?$ip:'').'/'.(!empty($dns)?$dns:''));
            }
        }
