 ...
 *
 *
 * Interface class of the CAS library
 * PHP Version 5
 *
 * @file     CAS/CAS.php
...
/**
 * CAS version. accessible for the user by CAS::getVersion().
 */
...
/**
 * CAS::service() error code on success
 */
define("PHPCAS_SERVICE_OK", 0);
/**
 * CAS::service() error code when the PT could not retrieve because
...
/**
 * CAS::service() error code when the service was not available.
 */
...
/**
 * CAS default language (when CAS::setLang() is not used)
 */
...
class CAS
{
     ...
     * @note Only one of the CAS::client() and CAS::proxy functions should be
     * called, only once, and before all other methods (except CAS::getVersion()
     * and CAS::setDebug()).
     */
    public static function client($server_version, $server_hostname,
        ...
        if (is_object(self::$_PHPCAS_CLIENT)) {
            CAS :: error(self::$_PHPCAS_INIT_CALL['method'] . '() has already been called (at ' . self::$_PHPCAS_INIT_CALL['file'] . ':' . self::$_PHPCAS_INIT_CALL['line'] . ')');
        }
        if (gettype($server_version) != 'string') {
            CAS :: error('type mismatched for parameter $server_version (should be `string\')');
        }
        if (gettype($server_hostname) != 'string') {
            CAS :: error('type mismatched for parameter $server_hostname (should be `string\')');
        }
        if (gettype($server_port) != 'integer') {
            CAS :: error('type mismatched for parameter $server_port (should be `integer\')');
        }
        if (gettype($server_uri) != 'string') {
            CAS :: error('type mismatched for parameter $server_uri (should be `string\')');
        }
    ...
    public static function setDebug($filename = '')
    {
        if ($filename != false && gettype($filename) != 'string') {
            CAS :: error('type mismatched for parameter $dbg (should be false or the name of the log file)');
        }
        if ($filename === false) {
            ...
            CAS :: trace('START CAS-' . PHPCAS_VERSION . ' ******************');
        }
    }
    ...
    public static function error($msg)
    {
        ...
        echo "<br />\n<b>CAS error</b>: <font color=\"FF0000\"><b>" . __CLASS__ . "::" . $function . '(): ' . htmlentities($msg) . "</b></font> in <b>" . $file . "</b> on line <b>" . $line . "</b><br />\n";
        CAS :: trace($msg);
        CAS :: traceEnd();
    ...
    {
        $dbg = debug_backtrace();
        CAS :: log($str . ' [' . basename($dbg[0]['file']) . ':' . $dbg[0]['line'] . ']');
    }
    ...
    */
    ...
    /**
     * This method is used to set the language used by CAS.
     *
    ...
    {
        if (!is_object(self::$_PHPCAS_CLIENT)) {
            CAS :: error('this method should not be called before ' . __CLASS__ . '::client() or ' . __CLASS__ . '::proxy()');
        }
        if (gettype($lang) != 'string') {
            CAS :: error('type mismatched for parameter $lang (should be `string\')');
        }
    ...
    */
    ...
    public static function setHTMLHeader($header)
    {
        ...
        if (gettype($header) != 'string') {
            CAS :: error('type mismatched for parameter $header (should be `string\')');
        }
    ...
    public static function setHTMLFooter($footer)
    {
        ...
        if (gettype($footer) != 'string') {
            CAS :: error('type mismatched for parameter $footer (should be `string\')');
        }
    ...
    */
    ...
    public static function setPGTStorage($storage)
    {
        CAS :: traceBegin();
        if (!is_object(self::$_PHPCAS_CLIENT)) {
            CAS :: error('this method should only be called after ' . __CLASS__ . '::proxy()');
        }
        if (!self::$_PHPCAS_CLIENT->isProxy()) {
            CAS :: error('this method should only be called after ' . __CLASS__ . '::proxy()');
        }
        if (self::$_PHPCAS_CLIENT->wasAuthenticationCalled()) {
            CAS :: error('this method should only be called before ' . self::$_PHPCAS_CLIENT->getAuthenticationCallerMethod() . '() (called at ' . self::$_PHPCAS_CLIENT->getAuthenticationCallerFile() . ':' . self::$_PHPCAS_CLIENT->getAuthenticationCallerLine() . ')');
        }
        if ( !($storage instanceof CAS_PGTStorage) ) {
            CAS :: error('type mismatched for parameter $storage (should be a CAS_PGTStorage `object\')');
        }
    ...
    /**
     * This method is used to tell CAS to store the response of the
    ...
    public static function setPGTStorageDb($dsn_or_pdo, $username='',
        ...
        if (gettype($username) != 'string') {
            CAS :: error('type mismatched for parameter $username (should be `string\')');
        }
        if (gettype($password) != 'string') {
            CAS :: error('type mismatched for parameter $password (should be `string\')');
        }
        if (gettype($table) != 'string') {
            CAS :: error('type mismatched for parameter $table (should be `string\')');
        }
    ...
    public static function setPGTStorageFile($path = '')
    {
        ...
        if (gettype($path) != 'string') {
            CAS :: error('type mismatched for parameter $path (should be `string\')');
        }
    ...
    */
    ...
    public static function getProxiedService ($type)
    {
        ...
        if (!self::$_PHPCAS_CLIENT->wasAuthenticationCalled()) {
            CAS :: error('this method should only be called after the programmer is sure the user has been authenticated (by calling ' . __CLASS__ . '::checkAuthentication() or ' . __CLASS__ . '::forceAuthentication()');
        }
        if (!self::$_PHPCAS_CLIENT->wasAuthenticationCallSuccessful()) {
            CAS :: error('authentication was checked (by ' . self::$_PHPCAS_CLIENT->getAuthenticationCallerMethod() . '() at ' . self::$_PHPCAS_CLIENT->getAuthenticationCallerFile() . ':' . self::$_PHPCAS_CLIENT->getAuthenticationCallerLine() . ') but the method returned false');
        }
        if (gettype($type) != 'string') {
            CAS :: error('type mismatched for parameter $type (should be `string\')');
        }
    ...
    public static function serviceWeb($url, & $err_code, & $output)
    {
        ...
        if (gettype($url) != 'string') {
            CAS :: error('type mismatched for parameter $url (should be `string\')');
        }
    ...
    public static function serviceMail($url, $service, $flags, & $err_code, & $err_msg, & $pt)
    {
        ...
        if (gettype($flags) != 'integer') {
            CAS :: error('type mismatched for parameter $flags (should be `integer\')');
        }
    ...
    */
    ...
    public static function setCacheTimesForAuthRecheck($n)
    {
        ...
        if (gettype($n) != 'integer') {
            CAS :: error('type mismatched for parameter $n (should be `integer\')');
        }
    ...
    public static function forceAuthentication()
    {
        ...
        /*		if (!$auth) {
         CAS :: trace('user is not authenticated, redirecting to the CAS server');
        self::$_PHPCAS_CLIENT->forceAuthentication();
        } else {
        CAS :: trace('no need to authenticate (user `' . CAS :: getUser() . '\' is already authenticated)');
        }*/
     ...
     * @warning should not be called only after CAS::forceAuthentication()
     * or CAS::checkAuthentication().
     * */
    public static function getUser()
    {
        ...
        if (!self::$_PHPCAS_CLIENT->wasAuthenticationCalled()) {
            CAS :: error('this method should only be called after ' . __CLASS__ . '::forceAuthentication() or ' . __CLASS__ . '::isAuthenticated()');
        }
        if (!self::$_PHPCAS_CLIENT->wasAuthenticationCallSuccessful()) {
            CAS :: error('authentication was checked (by ' . self::$_PHPCAS_CLIENT->getAuthenticationCallerMethod() . '() at ' . self::$_PHPCAS_CLIENT->getAuthenticationCallerFile() . ':' . self::$_PHPCAS_CLIENT->getAuthenticationCallerLine() . ') but the method returned false');
        }
    ...
    public static function setServerLoginURL($url = '')
    {
        CAS :: traceBegin();
        if (!is_object(self::$_PHPCAS_CLIENT)) {
            CAS :: error('this method should only be called after' . __CLASS__ . '::client()');
        }
        if (gettype($url) != 'string') {
            CAS :: error('type mismatched for parameter $url (should be `string`)');
        }
    ...
    public static function logout($params = "")
    {
        CAS :: traceBegin();
        if (!is_object(self::$_PHPCAS_CLIENT)) {
            CAS :: error('this method should only be called after ' . __CLASS__ . '::client() or' . __CLASS__ . '::proxy()');
        }
        $parsedParams = array ();
        if ($params != "") {
            if (is_string($params)) {
                CAS :: error('method `CAS::logout($url)\' is now deprecated, use `CAS::logoutWithUrl($url)\' instead');
            }
            if (!is_array($params)) {
                CAS :: error('type mismatched for parameter $params (should be `array\')');
            }
            foreach ($params as $key => $value) {
                if ($key != "service" && $key != "url") {
                    CAS :: error('only `url\' and `service\' parameters are allowed for method `CAS::logout($params)\'');
                }
    ...
    public static function logoutWithRedirectService($service)
    {
        ...
        if (!is_string($service)) {
            CAS :: error('type mismatched for parameter $service (should be `string\')');
        }
     ...
     * PGT. When this method is not called, a CAS script uses its own URL
     * for the callback.
     *
     ...
     * method is not called, a CAS script uses its own URL.
     *
     * @param string $url the URL
     *
     * @return void
    ...
    public static function retrievePT($target_service, & $err_code, & $err_msg)
    {
        ...
        if (gettype($target_service) != 'string') {
            CAS :: error('type mismatched for parameter $target_service(should be `string\')');
        }
    ...
    public static function setCasServerCACert($cert, $validate_cn = true)
    {
        ...
        if (gettype($cert) != 'string') {
            CAS :: error('type mismatched for parameter $cert (should be `string\')');
        }
        if (gettype($validate_cn) != 'boolean') {
            CAS :: error('type mismatched for parameter $validate_cn (should be `boolean\')');
        }
    ...
    public static function setNoCasServerValidation()
    {
        ...
        CAS :: trace('You have configured no validation of the legitimacy of the cas server. This is not recommended for production use.');
    ...
    public static function allowProxyChain(CAS_ProxyChain_Interface $proxy_chain)
    {
        ...
        if (self::$_PHPCAS_CLIENT->getServerVersion() !== CAS_VERSION_2_0) {
            CAS :: error('this method can only be used with the cas 2.0 protool');
        }
    ...
    {
        CAS::traceBegin();
        CAS::log('rebroadcastNodeUrl:'.$rebroadcastNodeUrl);
        if (!is_object(self::$_PHPCAS_CLIENT)) {
            CAS :: error('this method should only be called after ' . __CLASS__ . '::client() or' . __CLASS__ . '::proxy()');
        }
        if ( !(bool)preg_match("/^(http|https):\/\/([A-Z0-9][A-Z0-9_-]*(?:\.[A-Z0-9][A-Z0-9_-]*)+):?(\d+)?\/?/i", $rebroadcastNodeUrl)) {
            CAS::error('type mismatched for parameter $rebroadcastNodeUrl (should be `url\')');
        }
...
/** @defgroup internalBehave Internal behaviour of CAS
