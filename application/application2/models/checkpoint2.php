...
class Checkpoint2 extends CI_Model {
  ...
  function insert_entry($project_id)
  {
    ...
    $this->db->insert('checkpoint2', $this);
  }
  ...
  function update_entry($project_id)
  {
    ...
    $query = $this->db->query("SELECT * fROM project  WHERE student_netID = '$this->student_netID' AND semester = '$this->semester';");
    $row = $query->row();
    $id = $row->id;
    $this->db->update('checkpoint2', $this, array('id' => $id));
  }
}